...
const PricingPage = () => {
  ...
  useEffect(() => {
    if (tiers.length > 0 && selectedTier === null) {
      ...
      setSelectedTier(popular || tiers[0]);
    }
  }, [tiers, selectedTier]);
  ...
  const handlePurchase = () => {
    if (selectedTier) navigate('/purchase', { state: { tier: selectedTier } });
  };
          ...
          <>
            <div className="pp-grid">
              {tiers.map((tier, i) => (
                <PricingCard
                  key={i}
                  tier={tier}
                  isSelected={selectedTier && tier.name === selectedTier.name && tier.price === selectedTier.price}
            ...
            {selectedTier && (
              <div className="pp-cta-wrap">
                <button className="pp-cta-single" onClick={handlePurchase}>
                  Get {selectedTier.name} Plan — ${selectedTier.price}/{selectedTier.period || 'month'}
                </button>
              </div>
            )}
          </>
